...
export class Page1Service extends ShuttleStoreService {
  static APPTITLE_IDENTIFIER = ['app-title', Page1Service];
  static COLOR_IDENTIFIER = ['color', Page1Service];

  constructor(store: ShuttleStore) { super(store); }

  setTitle(title: string) { this.store.setState(title, P1S.APPTITLE_IDENTIFIER); }
...
export class Page1State {
  ...
  get titleReplayStream$$() { return this.store.getPresetReplayStream$<string>(P1S.APPTITLE_IDENTIFIER, null, 25); }
